...
function deleteLastOld(){
    if(last.hasClass('conclusion')){
        $(".conclusion").remove()
    }else{
        n-=2
        last.next().remove()
...
function addLine(){
    ...
    $("#formula").show();$("#formula").focus()                            //Not applicable for level 1
    $("#formula").on('keydown',formulaLineKeydown)  
    $("#formula").on('input',formulaPremiseOrLineInput)
    $("#done").show()
    $("#done").on('click',lineDone)                 //end not applicable for level 1
    $(".spButton").show()
    $("#cancelAdd").show()
