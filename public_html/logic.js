...
function messageOn(msg, $elems, func) {
    ...
    if ($("#msgRespEnter").length > 0) {
        $("#msgRespEnter").on("click", {
            'func': func
        }, msgResponse) //add a listener to the msgResponse button (if one was added in msg).
...
function myMsg() {
    ...
    messageOn(msg, $(".fol").find('.subs'), function (elem) {
        console.log(elem)
    })
}
